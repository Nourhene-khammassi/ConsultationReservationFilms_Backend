...
import com.example.demo.configimage.FileStorageProperties;
...
@Service("FilmImageStorageImpl")
public class FilmImageStorageImpl implements ImageStorage {
    private final Path imageLocation;

    @Autowired
   
    public FilmImageStorageImpl(FileStorageProperties fileStorageProperties) {
        this.imageLocation= Paths.get(fileStorageProperties.getUploadImgFilmDir()).toAbsolutePath().normalize();
        try {
    ...
    @Override
    public ResponseEntity<Resource> downloadFilmImage(String imageName, HttpServletRequest request) {
